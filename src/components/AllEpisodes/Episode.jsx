      ...
      >
        <div className="episode-pic-wrapper">
          <img
            ...
            src={
              movieItem.attributes?.thumbplay?.thumbplay_img_m
                ? movieItem.attributes?.thumbplay.thumbplay_img_m
                : movieItem.attributes.pic.movie_img_m
            }
            alt=""
          />
          <div className="episode-duration u500">
            {Number(movieItem.attributes.duration) === 0 && "به زودی"}
            {Number(movieItem.attributes.duration) !== 0 &&
              durationTimeFormat(Number(movieItem.attributes.duration))}
          </div>
          {movieItem.attributes.user_watched_info.is_watch && (
            <div className="episode-progressbar-wrapper">
              <div
                className="episode-progressbar"
                style={{
                  width: `${movieItem.attributes.user_watched_info.percent}%`,
                }}
              ></div>
            </div>
          )}
        ...
        <div>
          <div className="episode-header">
            <h3 className="u700">{movieItem.attributes.movie_title}</h3>
            {movieItem.attributes.user_watched_info.is_watch && (
              <div className="is-watched">
                ...
                {movieItem.attributes.user_watched_info.text}
              </div>
            )}
          </div>
          <p className="episode-desc u400">
            {movieItem.attributes.descr.slice(0, 150)} ...
          </p>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        <div className="episode-pic-wrapper">
          ...
          />
          <div className="episode-duration u500">
            {Number(movieItem.duration) === 0 && "به زودی"}
            {Number(movieItem.duration) !== 0 &&
              durationTimeFormat(Number(movieItem.duration))}
          </div>
