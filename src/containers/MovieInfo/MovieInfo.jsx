...
import HeroBadge from "@src/components/HeroBadge/HeroBadge";
...
import RecommPanel from "./tabs/RecommPanel";
...
function MovieInfo({ isLogin }) {
  ...
  useEffect(() => {
    if (movieData) {
      ...
      if (!activeTab) {
        setActiveTab(
          movieData?.data?.General?.serial?.enable ? "episodes" : "detail",
        );
      }
    }
  }, [movieData]);
  ...
  const handleBack = useCallback(
    (e) => {
      ...
      // On the tab strip -> scroll back up and return to the play button.
      if (currentKey.startsWith("movieinfo-tab-")) {
        e.preventDefault?.();
        e.stopPropagation?.();
        setTabsActive(false);
      ...
      // Inside a tab's content panel -> return to the tab strip.
      if (tabsActive && activeTab && currentKey !== "paly-btn") {
        e.preventDefault?.();
        e.stopPropagation?.();
        setFocus(tabFocusKey(activeTab));
        return;
      }

      // Top controls -> leave the page.
      uiStorage.removeItem("lastSeasonFocus");
      uiStorage.removeItem("lastSeasonFocus_parent_new");
      uiStorage.removeItem("lastSeasonFocus_season_part");
    ...
    [tabsActive, activeTab, location.pathname, navigate],
  );

  useBackKey(handleBack, { capture: true });
  ...
  // Map the scroll position of .hero-scroll-content to a 0..1 progress value
  // exposed as --scroll-progress on the page root. The hero gradient fades out
  // and the dim overlay fades in as the tab strip is pulled up to the top.
  const handleScroll = () => {
    const el = scrollRef.current;
    if (!el) return;
    const max = el.scrollHeight - el.clientHeight;
    const p = max > 0 ? Math.min(1, Math.max(0, el.scrollTop / max)) : 0;
    ref.current?.style.setProperty("--scroll-progress", p);
    setTabsActive(p > 0.9);
  };
  ...
  const handlePlayArrowPress = (direction) => {
    if (direction === "down" && activeTab) {
      // The panel height is clamped so scrollHeight lands exactly at tabs-pinned.
  ...
  const handleTabFocus = (key) => {
    setActiveTab(key);
    setTabsActive(true);
    // Pin the tab strip to the top (same as arrowing down from the play
    // button) so clicking a tab lifts the hero up instead of leaving it
    // mid-scroll. Harmless when already pinned.
    scrollRef.current?.scrollTo({
      top: scrollRef.current.scrollHeight,
      behavior: "smooth",
    });
  };
  const handleTabExitUp = () => {
    setTabsActive(false);
    scrollRef.current?.scrollTo({ top: 0, behavior: "smooth" });
    setFocus("paly-btn");
  };
  ...
  const handleContentExitUp = () => {
    setTabsActive(true);
    if (activeTab) setFocus(tabFocusKey(activeTab));
  };
  ...
  const general = movieData?.data?.General;
  const uid = general?.uid;
  const isSerial = general?.serial?.enable;
  const tabs = [
    { key: "detail", label: isSerial ? "اطلاعات سریال" : "اطلاعات فیلم" },
    ...(isSerial ? [{ key: "episodes", label: "قسمت‌های سریال" }] : []),
    { key: "recomm", label: "فیلم‌های مشابه" },
  ];

  return (
    <FocusContext.Provider value={focusKey}>
      {!isLoading && (
        <main id="movieinfo-page" ref={ref} className="main">
          {isShowAlert && (
            <Alert
          ...
          <div className="hero movie-info-hero">
            ...
            <div className="movieinfo-overlay" />
            {isImageLoaded ? (
              <img
            ...
            >
              <div className="hero-content">
                <div className="hero-text">
                  {movieData?.data?.General?.movie_logo ? (
                    <img
                      className="hero-text-logo"
                      src={movieData.data.General.movie_logo}
                      alt={
                        movieData?.data?.General?.movie_name ||
                  ...
                  movieData?.data?.General?.exclusive?.is_active ? (
                    <div className="schedule">
                      {movieData?.data?.General?.exclusive?.is_active && (
                        <>
                          <div className="calender calender-exclusive">
                            <img
                              className="calender-logo"
                              src={movieData?.data?.General?.exclusive.icon}
                              alt="اختصاصی"
                            />
                          </div>
              ...
              <div className="movie-info-down-section">
                {activeTab && (
                  <TabBar
                    tabs={tabs}
                    activeTab={activeTab}
                ...
                {activeTab && (
                  <div className="movieinfo-tabpanel">
                    {activeTab === "episodes" && (
                      <EpisodesPanel
                        ref={panelRef}
                        uid={uid}
