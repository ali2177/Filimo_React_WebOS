...
function Profile() {
  ...
  const [isLoading, setIsLoading] = useState(!!jwt);
  ...
  const profile = UserData?.data?.attributes;
  ...
  if (error || !profile) {
    return (
      <main className="main info">
        <p className="u500">مشکلی در دریافت اطلاعات حساب کاربری پیش آمد.</p>
      </main>
    );
  }
    ...
    <FocusContext.Provider value={focusKey}>
      <main ref={ref} className="profile">
        ...
        <div className="prifile-detail">
          <div className="user">
            <p className="user-email-label u500">آدرس ایمیل :</p>
            <span className="user-data u500">
              {profile?.profile_info?.email}
            </span>
          </div>
        </div>
        <div className="subscription">
          <div className="subscription-text">
            <h4 className="subscription-text-header u700">وضعیت اشتراک</h4>
            <div className="subscription-status">
              <svg
                className="subscription-status-svg"
              ...
              <p className="subscription-status-text u500">
                {profile?.profile_state_info?.descr}
              </p>
            </div>
          </div>

          <div className="subscription-notif u500">
            {profile?.profile_state_info?.pay_desc}
          </div>
        </div>
        <div className="subscription">
          <div className="subscription-text">
            <div className="subscription-status">
              <div className="profile-pic-wrraper">
                <img
                  ...
                  src={profile?.profile_icon_info}
                  alt="تصویر پروفایل"
                />
              </div>
              <div className="user-info">
                <p className="u500">{profile?.profile_info?.name}</p>
                <div className="user-info-id">
                  <svg
                    className="user-info-svg"
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                  ...
                  <p className="u500">{profile?.profile_info?.id}</p>
                </div>
              </div>
            </div>
          </div>
